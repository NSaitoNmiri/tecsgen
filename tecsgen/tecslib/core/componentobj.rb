    ...
    if( cell && cell.get_allocator_list ) then      # cell == nil なら既にエラー
      ...
      cell.get_allocator_list.each { |a|
        ...
        if elements[0] == :OP_SUBSC then
          subscript = elements[2].eval_const(nil) # 受け口の配列添数：:OP_SUBSC の第二引数
        else
          subscript = nil
        end
...
##    if Generator.get_plugin then     # mikan これは必要？ (意味解析段階での実行になるので不適切)
  ...
  # その後、このメソッドが呼ばれる
  # mikan Region#distance とRegion へたどり着くまでための処理が共通
...
# p @to_through_list
  ...
  # mikan Cell#check_and_gen_through とRegion へたどり着くまでための処理が共通
