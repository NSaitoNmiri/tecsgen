...
EOT
  ...
  def generate_cell_code
    ...
    gen_cell_cb_out_init f         # INITIALIZE_CB で参照されるため ram_initializer=false でも消せない
    gen_cell_cb f
    gen_cell_ep_des f
    if $ram_initializer then
...
#    }
  ...
  def gen_ph_get_cellcb f
    ...
    elsif @idx_is_id then   # mikan 単一のセルの場合の最適化, idx_is_id でない場合
...
#####  celltype glue code
          ...
          if am  then
            ...
            while( i < am.length )
              ...
              if j then
                ...
                # 右辺の Expression から式の要素を得る
                elements = e.get_elements

                # 右辺は受け口配列か？
                if elements[0] == :OP_SUBSC then

                  # 受け口の配列添数： :OP_SUBSC の第二引数
                  subscript = elements[2].eval_const(nil)

                  # 受け口 elements[0] は :OP_DOT
                  elements = elements[1]             # mikan elements[2]: 配列要素
  ...
  def gen_cell_ep_vdes_array f
    if @n_cell_gen >0 then
      f.printf( TECSMsg.get( :CPA_comment ), "#_CPA_#" )   # mikan 呼び口配列が無い場合も出てしまう
    end
    ...
    if @n_call_port != 0 then
      ...
      port.each{ |p|
        ...
        if am then
          ...
          if elements[0] == :OP_SUBSC then    # mikan elements 撲滅
            # 受け口配列の場合
            subscript = elements[2].eval_const(nil)
            elements = elements[1]           # mikan elements[2]: 配列要素
